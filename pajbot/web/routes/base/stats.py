...
def init(app):
    ...
    def stats():
        ...
        with DBManager.create_session_scope() as db_session:
            top_5_line_farmers = (
                db_session.query(User).filter_by(ignored=False).order_by(User.num_lines.desc()).limit(5).all()
            )
            top_5_paid_timeouts = (
                db_session.query(User).filter_by(ignored=False).order_by(User.num_paid_timeouts.desc()).limit(5).all()
            )
            return render_template(
                "stats.html",
                top_5_commands=top_5_commands,
                top_5_line_farmers=top_5_line_farmers,
                top_5_paid_timeouts=top_5_paid_timeouts,
            )
