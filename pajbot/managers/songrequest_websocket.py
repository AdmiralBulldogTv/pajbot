...
class MyServerProtocol(WebSocketServerProtocol):
    ...
    def onMessage(self, payload, isBinary):
        with DBManager.create_session_scope() as db_session:
            if not isBinary:
                ...
                try:
                    if not self.manager.bot.songrequest_manager.states["enabled"]:
                        self._close_conn()
                        return
                ...
                if alert:
                    self.sendMessage(json.dumps({"event": "alert_message", "data": alert}).encode("utf8"), False)
    ...
    def _pause(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.pause_function()
    ...
    def _resume(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.resume_function()
        except (ManagerDisabled, InvalidState):
            return 0, None
    ...
    def _ready(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.ready_function()
        except (ManagerDisabled, InvalidState):
            return 0, None
    ...
    def _show_video(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.show_function()
        except (ManagerDisabled, InvalidState):
            return 0, None
    ...
    def _hide_video(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.hide_function()
        except (ManagerDisabled, InvalidState):
            return 0, None
    ...
    def _request_state(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.request_state_function(data["value"])
        except (ManagerDisabled, InvalidState):
            return 0, None
    ...
    def _auto_play_state(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.auto_play_state_function(data["value"])
        except (ManagerDisabled, InvalidState):
            return 0, None
    ...
    def _backup_playlist_state(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.backup_playlist_state_function(data["value"])
        except (ManagerDisabled, InvalidState):
            return 0, None
    ...
    def _use_spotify_state(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.use_spotify_state_function(data["value"])
        except (ManagerDisabled, InvalidState):
            return 0, None
    ...
    def _play_on_stream_state(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.play_on_stream_function(data["value"])
        except (ManagerDisabled, InvalidState):
            return 0, None
    ...
    def _next(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.skip_function(self.login)
    ...
    def _previous(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.previous_function(self.login)
        except (ManagerDisabled, UserNotFound):
            return 0, None
    ...
    def _seek(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.seek_function(data["seek_time"])
    ...
    def _volume(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.volume_function(int(data["volume"]))
    ...
    def _move(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.move_function(int(data["database_id"]), int(data["to_id"]) - 1)
        except (ManagerDisabled, InvalidSong):
            return 0, None
    ...
    def _favourite(self, db_session, data):
        if (
            not self.isAuthed
            or not data
            or ("database_id" not in data and "songinfo_database_id" not in data and "hist_database_id" not in data)
        ):
            return 0, None

        try:
            self.manager.bot.songrequest_manager.favourite_function(
    ...
    def _unfavourite(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.unfavourite_function(
                database_id=data.get("database_id", None),
                hist_database_id=data.get("hist_database_id", None),
                songinfo_database_id=data.get("songinfo_database_id", None),
            )
    ...
    def _request(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.request_function(
    ...
    def _ban(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.ban_function(
                database_id=data.get("database_id", None),
                hist_database_id=data.get("hist_database_id", None),
                songinfo_database_id=data.get("songinfo_database_id", None),
            )
    ...
    def _unban(self, db_session, data):
        if (
            ...
            return 0, None
        try:
            self.manager.bot.songrequest_manager.unban_function(
                database_id=data.get("database_id", None),
                hist_database_id=data.get("hist_database_id", None),
                songinfo_database_id=data.get("songinfo_database_id", None),
            )
    ...
    def _delete(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.remove_function(int(data["database_id"]))
        except (ManagerDisabled, InvalidSong):
            return 0, None
    ...
    def _add_media(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.request_function(requested_by=self.login, video_id=youtube_id)
        except SongBanned:
            return (
                1,
                {"success": False, "header": "That song is currently banned", "text": "", "duration": 2000},
    ...
    def _set_backup_playlist(self, db_session, data):
        ...
        try:
            self.manager.bot.songrequest_manager.set_backup_playlist(playlist_id)
    ...
    def _auth(self, db_session, data):
        ...
        user = self.manager.bot.twitch_v5_api.user_from_access_token(
            access_token, self.manager.bot.twitch_helix_api, db_session
        )
    ...
    def _dump_state(self, db_session):
        current_song = self.manager.bot.songrequest_manager.current_song
        data = {
            "volume": self.manager.bot.songrequest_manager.volume,
            "current_song": current_song.webjsonify() if current_song else {},
            "module_state": self.manager.bot.songrequest_manager.states,
